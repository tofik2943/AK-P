...
{
	float alpha = 0;
	const int arraySize = 21; //7punktów po 3 wspó³rzêdne
...
enum cudaMemcpyKind 	kind
	...
	*/
	/*
	cudaStatus = cudaMemcpy(dev_c, c, arraySize * sizeof(float), cudaMemcpyHostToDevice);		//nie wiem czy ³adowanie wartoœci poprzedniego elementu tablicy C ma sens
	if (cudaStatus != cudaSuccess) {
	...
	*/
	...
	addKernel << <1, 7 >> > (c, tab, alpha);															//   <<<ile bloków , ile w¹tków w bloku>>> ?
